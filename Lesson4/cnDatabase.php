<?php 	
	...
	$queryPostsRand4 = "select posts.*,categories.title as 'category' from posts, categories where posts.category_id = categories.id order by rand() limit 4";
	$qPostsRand4 = $conn->query($queryPostsRand4);
	$postsRand4 = array();
	while ($row = $qPostsRand4->fetch_assoc()) {
		$postsRand4[] = $row;
	}

	//Hiện thị số bài viết theo từng category
	$queryCountCategory = "select categories.id, categories.title, count(posts.category_id) as 'soluong' from categories inner join posts on categories.id = posts.category_id
		group by categories.id";
	$qCountCategory = $conn->query($queryCountCategory);
	$countCategory = array();
	while ($row = $qCountCategory->fetch_assoc()) {
		$countCategory[] = $row;
	}
